...
export class PerfilModel {
  static async buscarPerfilAdmin({
    page = 1,
    limit = 12,
    busca = "",
    ordem = "",
    ...
    let query = supabaseAdmin
      .from("users_profile")
      .select(
        "id, nome, funcao , is_admin ,telefone, imagem, descricao, redes_sociais, is_admin, transicao_de_status,livros (ativo, estado, titulo, capa, id), revisoes(data_criacao, apontamento, fk_livro_id, nome)",
        { count: "exact" },
      );
    ...
    if (funcao === "funcionario") {
      ...
      query = supabaseAdmin
        .from("users_profile")
        .select(
          "id, nome, funcao , telefone, imagem, descricao, redes_sociais, is_admin, transicao_de_status, livros!inner(ativo, estado, titulo, capa, id), revisoes(data_criacao, apontamento, fk_livro_id, nome)",
          {
            count: "exact",
          },
        )
