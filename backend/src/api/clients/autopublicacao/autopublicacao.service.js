...
export const getLivrosByIdService = async (userId) => {
  if (!userId) {
  ...
  if (!data || data.length === 0) {
    const error = new Error("Nenhum rascunho de livro encontrado para este autor.");
...
export const updateEstadoService = async (livroId) => {
  await AutopublicacaoModel.deixarRascunho(livroId);
};

export const insertLivroService = async ({ userId, dadosLivro = {}, publicar = true, capa = {}, manuscritoPath = null }) => {
  if (!userId) {
    const error = new Error("Sessão expirada. Autentique-se novamente para publicar.");
    error.statusCode = 401;
    throw error;
  }
  ...
  if (manuscritoPath) {
    ...
    if (!manuscritoPath.startsWith(caminhoEsperado)) {
      const error = new Error("Tentativa inválida de manipulação de arquivo de outro usuário.");
...
export const criarUploadLivroService = async ({ userId, tipo, extensao }) => {
  if (!userId) {
