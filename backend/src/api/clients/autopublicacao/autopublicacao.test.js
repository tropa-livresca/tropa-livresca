...
app.get("/api/v1/clients/autopublicacao", AutopublicacaoController.GetLivrosById);
app.post("/api/v1/clients/autopublicacao/upload-url", AutopublicacaoController.CriarUploadLivro);
app.post("/api/v1/clients/autopublicacao/insertLivro", AutopublicacaoController.InsertLivro);
app.patch("/api/v1/clients/autopublicacao/updateEstado/:id", AutopublicacaoController.UpdateEstado);
...
test("Suíte de Testes do AutopublicacaoController", async (t) => {
  await t.test("GetLivrosById - Deve retornar a lista de livros com capas do usuário", async () => {
    const listaMock = [
    ...
    const response = await request(app)
      .get("/api/v1/clients/autopublicacao");

    assert.strictEqual(response.status, 200);
    assert.deepStrictEqual(response.body, listaMock);
  });

  await t.test("CriarUploadLivro - Deve gerar uma URL de upload com sucesso", async () => {
  ...
  await t.test("InsertLivro - Deve cadastrar um novo livro e retornar os dados salvos", async () => {
    const payload = {
  ...
  await t.test("UpdateEstado - Deve atualizar o estado do livro via PATCH e retornar 200", async () => {
    let idRecebido = null;
    AutopublicacaoService.updateEstadoService = async (id) => {
      idRecebido = id;
    };
