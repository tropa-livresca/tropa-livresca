    ...
    <main className={styles.container}>
            ...
            <div key={livro.id}>
              ...
              <div>
                <button className={`${styles.btn} ${styles.btnVisualizar}`}>
                  <Link to={`/visualizar-livro/${livro.id}`}> Visualizar </Link>
                </button>
                <Link to={`/editar-livro/${livro.id}`} className={`${styles.btn} ${styles.btnEditar}`}>
                  Editar
                </Link>
                {livro.estado === "rascunho" && (
                  <button onClick={() => updateEstado(livro.id, "em_revisao")} className={`${styles.btn} ${styles.btnPublicar}`}>
                    Enviar para Revisão
                  </button>
                )}
                {livro.estado === "em_revisao" && (
                  <button onClick={() => updateEstado(livro.id, "rascunho")} className={`${styles.btn} ${styles.btnPublicar}`}>
                    Cancelar Revisão (Voltar para Rascunho)
                  </button>
                )}
                {livro.estado === "publicado" && <span> Publicado </span>}
                {livro.estado !== "em_revisao" && (
                  <button
      ...
      {!carregando && meta && meta.totalPages > 1 && (
        <div>
          ...
          <button onClick={() => setPaginaAtual((prev) => Math.min(prev + 1, meta.totalPages))} disabled={paginaAtual === meta.totalPages}>
            Próximo
          </button>
        </div>
      )}
